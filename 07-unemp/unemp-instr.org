...
- Estime por mínimos cuadrados ordinarios, MCO, los parámetros de:
  ...
  de crecimiento porcentual del PIB real y $v_t$ es el término de
  error.
...
- Estime por MCO los parámetros de:
  ...
  \begin{equation}\label{eq:2}
    \Delta \unemp_t = \alpha + \beta\, \gY_t + \delta\, \covid + u_t,
  \end{equation}
  #+end_export
...
- Para calcular el multiplicador de largo plazo, $\Theta = \sum
  ...
  regresión.]/
- Los multiplicadores acumulados, $\theta_j = \sum_{i=0}^j\beta_i$, se
...
- En [cite:@leon-ledesmaEndogeneityNaturalRate2002] se define la /tasa
  ...
  \unemp_t = 0$, por lo que $\gY_n = -\alpha / \Theta$. Esta tasa se
  puede obtener en la consola de Gretl con:
  #+latex: \vspace*{1ex}
  #+begin_src gretl :eval no
        gYn = -$coeff[1] / mult_lp
  #+end_src
