...
Se investigará la existencia de la relación entre inflación y
desempleo en España durante el periodo 1965--2023. Se estimarán
distintas especificaciones de la curva de Phillips.
...
- =paroc=: tasa de paro cíclico calculada a partir de la aplicación
  del filtro de Hodrick-Prescott ($\lambda = 6.25$ xes el valor
  comunmente utilizado para datos anuales) a la tasa de desempleo
  observada. En Gretl, se obtiene añadiendo una nueva variable
  definida como =paroc = hpfilt(paro, 6.25)=.
...
=d_infl=, =paro= y =paroc=. En todos los casos:

#+ATTR_LATEX: :options [a)]
1. Determine, a partir de la inspección del gráfico de la serie
   temporal, si es necesario incluir una tendencia lineal en la
...
#+begin_answer
  ...
  No se observan claras tendencias crecientes o decrecientes a lo
  largo de todo el periodo en los gráficos de la inflación, =infl=, la
  diferencia de la inflación, =d_infl=, la tasa de paro observada,
  ...
  regresiones del contraste. De acuerdo con el AIC, no se incluye
  ningún retardo en los casos de la inflación y su primera diferencia,
  mientras que se selecciona 1 retardo en el caso de =paro= y 3
  retardos en el caso de =paroc=.

  Los resultados de los contrastes no permiten rechazar, al 5% de
  significación, la existencia de una raíz unitaria en las series
  =infl= y =paro=. La hipótesis de una raíz unitaria se rechaza en los
  casos de la primera diferencia de la inflación, =d_infl=, y del paro
  cíclico, =paroc=. Por tanto, la inflación y el paro parecen ser
  procesos integrados de orden uno, mientras que la inflación en
  diferencias y el paro cíclico son I(0).
#+end_answer
...
- Estime una curva de Phillips estática presentada en la ecuación
  ...
  #+begin_answer
    ...
    El coeficiente estimado del paro es negativo y estadísticamente
    significativo. Esto indica que un aumento de un punto porcentual
    en la tasa de paro se asocia, en promedio, con una reducción de
    aproximadamente $0.33$ puntos porcentuales en la inflación.
  #+end_answer
...
- Atendiendo a los resultados obtenidos con los contrastes de raíces
  ...
  #+begin_answer
    Las variables =infl= y =paro= presentan raíces unitarias por lo
    que se incumple el supuesto *ST.1'*, que requiere la
    estacionariedad y la dependencia débil de las series incluidas en
    la regresión.  Dado que ambas variables parecen tener raíz
    unitaria, la estimación en niveles podría producir una regresión
    espuria.
  #+end_answer
...
- Obtenga los errores típicos robustos a heteroscedasticidad y
  ...
  #+begin_answer
    El valor del BIC se minimiza cuando se incluye 1 retardo en el
    modelo. Por tanto, el modelo seleccionado incluye el paro cíclico
    contemporáneo y un retardo.

    #+latex: \gretl{phillips-lags.txt}

    #+latex: \gretl{phillips-dyn.txt}

    El coeficiente contemporáneo del paro cíclico es negativo,
    mientras que el retardo es positivo. Esto sugiere que una
    reducción del paro cíclico aumenta inicialmente la inflación, pero
    parte de ese efecto se revierte en el periodo siguiente.
  #+end_answer
...
- Con los resultados de la regresión anterior, contraste al $5\%$ la
  ...
  #+begin_answer
    ...
    estimación del multiplicador de largo plazo, $\hat{\delta}_1 =
    0.09$, y de su error típico, $\hat{\sigma}_{\hat{\delta}_1} =
    0.13$. El estadístico $t$ resultante es $0.66$ y el valor $p =
    0.51$, por lo que no se rechaza al 5% la hipótesis de que el
    multiplicador de largo plazo es igual a 0.

    La siguiente tabla muestra la estimación del modelo bajo la
    hipótesis nula de ausencia de efectos de largo plazo del paro
    cíclico sobre la inflación.

    #+latex: \gretl{phillips-lr-mult-0.txt}

    En conjunto, los resultados sugieren que no existe una relación
    estable de largo plazo entre inflación y desempleo en España
    durante el periodo analizado. Sin embargo, sí se observan efectos
    de corto plazo del desempleo cíclico sobre los cambios en la
    inflación.
  #+end_answer
...
#  LocalWords:  Hodrick Prescott Fuller Dickey BIC Schwarz
