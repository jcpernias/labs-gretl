...
#+begin_answer
  ...
  lineal en las regresiones auxiliares de estas dos variables. El
  número óptimo de retardos se determina con el AIC, incluyéndose un
  retardo en las regresiones de =ly= y =lx= y ninguno en las de =gy= y
  =gx=. En los casos de =ly= y =lx= no es posible rechazar la
  hipótesis nula de raíz unitaria al \pct{5}. En los casos de =gy= y
  =gx= se rechaza la presencia de raíces unitarias al \pct{5}.
...
#+begin_answer
  ...
  plazo es significativo al nivel de significación del \pct{5}.
#+end_answer
...
#+begin_answer
  ...
  que los parámetros del modelo de regresión permanecen constantes a
  lo largo de todo el periodo. En concreto, los resultados de la
  estimación mostrarían que las exportaciones tendrían un efecto
  significativo únicamente en el periodo posterior a la crisis
  financiera de 2008.
#+end_answer
...
#  LocalWords:  PIB COVID autocorrelación
