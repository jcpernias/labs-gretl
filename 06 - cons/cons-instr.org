...
trimestrales de los años 1970--1998.
...
** Transformación y de las variables

- Cree la variable ~c~ tomando el logaritmo del consumo privado,
  ~PCR~.

- Cree la variable ~y~ calculando el logaritmo de la renta disponible
  de las familias, ~PYR~.
...
- Calcule los estadísticos descriptivos de las series ~c~ e ~y~. ¿Son
  representativas las medias de estas variables?
...
pueden producir resultados aparentemente significativos, aún cuando no
exista ninguna relación causal entre las variables. Podemos ilustrar
esta situación reemplazando en la ecuación \eqref{eq:1} la variable
...
- Considere ahora el modelo de regresión:
  ...
  Estime los parámetros de la ecuación \eqref{eq:2} por MCO, examine
  la bondad del ajuste de la regresión y compruebe la significación de
  la variable $\mathit{rw}_t$.


* Modelo en diferencias

- Calcule y represente gráficamente la primera diferencia de las
  variables, ~c~ e ~y~. ¿Son estacionarias las variables en
  diferencias?
...
- Obtenga un gráfico de series temporales de los residuos de la
